...
export class StartlijstPageComponent implements OnInit, OnDestroy {
  ...
  filter(aan: boolean) {
    ...
    if (!aan) {   // filtet staat uit
      ...
      this.filteredStarts = this.data.filter((s:HeliosStartDataset) => { return s.STARTTIJD == null});
      this.startExtendedVelden();
      this.filteredAanwezigVliegtuigen = this.aanwezigVliegtuigen.filter((a:HeliosAanwezigVliegtuigenDataset) => { return !a.VERTREK && a.VLIEGT == false});
